...
const idempotencyMiddleware = async (req, res, next) => {
  ...
  const holdingTime = 60; // 1 minute
  try {
    // Check if this key has been processed before
    // let processedRequest = memoryIdempotencyStore.get(uniqueKey);
    let processedRequest = await req.app?.cache?.query({ text: uniqueKey, values: [] }) || undefined;

    if (processedRequest && processedRequest.rows && processedRequest.rows.length > 0) {
      processedRequest = processedRequest.rows[0];
      // Return the cached response
      return (processedRequest.isPlain)
        ? res.status(processedRequest.status).send(processedRequest.body)
        : res.status(processedRequest.status).json(processedRequest.body);
    }
    ...
    await req.app?.cache?.buildCache(
      { text: uniqueKey, values: [] },
      { rows: [responseData], count: 1, ttl: holdingTime },
      holdingTime
    );
